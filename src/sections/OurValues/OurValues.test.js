...
jest.mock("antd", () => ({
	Steps: jest.fn(({ className, direction, progressDot, current, items }) => (
		<div
			data-testid="steps"
...
describe("OurValues", () => {
	...
	it("should render with correct background, container, title, and steps", () => {
		...
		expect(steps.dataset.classname).toBe(OUR_VALUES_STEPS_CLASS_NAME);
		expect(steps.dataset.direction).toBe(OUR_VALUES_STEPS_DIRECTION);
		expect(steps.dataset.progressdot).toBe("true");
		expect(Number(steps.dataset.current)).toBe(OUR_VALUES_ALL_STEPS.length);
		...
		OUR_VALUES_ALL_STEPS.forEach((step, i) => {
			...
			expect(stepEl.textContent).toContain(step.description);
		});
	});
});