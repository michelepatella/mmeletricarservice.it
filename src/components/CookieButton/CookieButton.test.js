...
describe("CookieButton", () => {
	...
	it("renders the button with correct aria-label", () => {
		render(<CookieButton setIsCookiesBannerVisible={jest.fn()} />);
		const button = screen.getByRole("button", { name: /cookie/i });
	...
	it("calls onCookieButtonClick handler when clicked", () => {
		...
		render(<CookieButton setIsCookiesBannerVisible={mockSetIsVisible} />);
		const button = screen.getByRole("button", { name: /cookie/i });
        
		fireEvent.click(button);

		expect(onCookieButtonClick).toHaveBeenCalledWith(mockSetIsVisible);
	});
});