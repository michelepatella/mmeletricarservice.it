...
describe("BlockQuote", () => {
     ...
     */ 
	it("renders children", () => {
		render(<BlockQuote>Some text within block quote</BlockQuote>);
		expect(screen.getByText("Some text within block quote")).toBeInTheDocument();
	});
	...
	it("renders a blockquote element", () => {
		...
		expect(screen.getByText("Some text within block quote").closest("blockquote")).toBeInTheDocument();
	});
});