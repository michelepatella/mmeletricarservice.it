...
import handler from "./usedCarsOverview.js";
...
/**
 * Test suite for the handler function in usedCarsOverview.js.
 * This suite includes:
 ...
 */
describe("usedCarsOverview handler", () => {
  ...
  it("should return used cars overview with status 200 if successful", async () => {
    ...
    expect(getUsedCarData).toHaveBeenCalledWith(USED_CAR_OVERVIEW_TABLE);
    expect(getUsedCarImages).toHaveBeenCalledWith("1", USED_CAR_OVERVIEW_IMAGE_LIMIT);
    expect(getUsedCarImages).toHaveBeenCalledWith("2", USED_CAR_OVERVIEW_IMAGE_LIMIT);

		// Assertions about the response
		expect(pLimit).toHaveBeenCalled();
		expect(pLimit).toHaveBeenCalledWith(P_LIMIT_CONCURRENCY);
  ...
  it("should handle empty images gracefully", async () => {
    ...
    getUsedCarData.mockResolvedValueOnce(carsData);
    getUsedCarImages.mockRejectedValueOnce(new Error("Something went wrong"));
